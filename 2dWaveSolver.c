...
int main(){
	...
	/* The first iteration with 1 thread is used exclusively to export solution data to files,
	  	...
	  	#pragma omp parallel private(i,j,n) shared(x,dx,y,dy,t,dt,u,u_1,u_2,r,Nx,Ny,Nt,g)
	  	{
			...
			for(i=0;i<Nx;i++) {
		       for(j=0;j<Ny;j++){
					...
					if (w==0) /* Export numerical and analytical solution data to file for t = 0 */
					{
