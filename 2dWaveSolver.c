...
int main(){
	...
	double	T = 20;
	int Nt = 250;  /* Time points */

	double r[Nx][Ny];
	
	FILE *f0,*f5,*f10,*f20,*fspeed,*feff; // Data files for t = 0, 5, 10, 20 and parallel speedup, efficiency.
	...
	int thread_num[] = {1,1,2,4,8}; /* Number of threads to be used */
	...
	for (w=0;w<5;w++) /* The first iteration with 1 thread is used exclusively to export solution data to files,
						 so the total time passed is ignored. Parallel speedup and efficiency are not being measured for this case */


	{

		
		omp_set_num_threads(thread_num[w]); /* Set number of threads */
	  	...
	  	#pragma omp parallel private(i,j,n) shared(x,dx,y,dy,t,dt,u,u_1,u_2,r,Nx,Ny,Nt,g)
	  	{
			...
			for(i=0;i<Nx;i++) {
		       for(j=0;j<Ny;j++){
					...
					if (w==0)
					{
					fprintf(f0, "%f %f %f %f \n\n", x[i],y[j],u_1[i][j],r[i][j]); /* Export numerical and analytical solution data to file for t = 0 */
					}
		   	...
		   	for(n=1;n<Nt;n++){
		   		...
		   		for(i=1;i<Nx;i++) {
		    		...
		    		for(j=1;j<Ny;j++){
		    			...
		    			u[i][j]= pow(dt,2)*f(x[i],y[j],t[n]) + g*(u_1[i+1][j]+u_1[i][j+1]
		       		    + u_1[i-1][j]+u_1[i][j-1]) + 2*u_1[i][j]*(1-2*g)
		       			-u_2[i][j];
		    	...
		    	#pragma omp for
			 	for(i=0;i<Nx;i++) {
		       	...
		       	// Export numerical and analytical solution data to different files corresponding to t = 5, t = 10 and t = 20
	   	...
	   	// Define central value, elapsed time, parallel speedup, parallel efficiency for each thread number case
    ...
    // Export speedup and efficiency data, along with corresponding thread number, to files
